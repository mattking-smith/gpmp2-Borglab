...
class BiasOmegaFactor
    : public gtsam::NoiseModelFactor2<gtsam::imuBias::ConstantBias, gtsam::Vector> {
private:
  double omega_z_;

  // typedefs
  typedef BiasOmegaFactor This;
  typedef gtsam::NoiseModelFactor2<gtsam::imuBias::ConstantBias, gtsam::Vector> Base;
...
public:
   ...
   */
  BiasOmegaFactor(gtsam::Key biasKey, gtsam::Key velKey, double omega_z, double cost_sigma)
      : Base(gtsam::noiseModel::Isotropic::Sigma(1, cost_sigma), biasKey, velKey),omega_z_(omega_z) {}
  ...
  gtsam::Vector
  evaluateError(const gtsam::imuBias::ConstantBias &bias, const gtsam::Vector &vel,
