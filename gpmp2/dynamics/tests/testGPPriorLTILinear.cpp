...
TEST(GPPriorLTILinear, Factor) {
  ...
  GPPriorLTILinear factor(key_pose1, key_vel1, key_pose2, key_vel2, key_control,
                          delta_t, Qc_model);
  Vector3 p1, p2;
  Vector3 v1, v2, c1;
  ...
  p1 = (Vector3() << -0.1, 1.2, 0.3).finished();
  v1 = (Vector3() << 0, 0, 0).finished();
  c1 = (Vector3() << 1, 3, 5).finished();

  /*delta_t = 0.01 */
  // p2 = (Vector3() << -0.0999998780487805, 1.20000036585366,
  // 0.300008771929825).finished(); v2 = (Vector3()
  // << 2.4390243902439e-05, 7.31707317073171e-05,
  // 0.00175438596491228).finished();

  /*delta_t = 0.5 */
  p2 = (Vector3() << -0.0996951219512195, 1.20091463414634, 0.321929824561404)
           .finished();
  v2 =
  ...
  expectH1 = numericalDerivative11(
      std::function<Vector(const Vector3 &)>(std::bind(
          &GPPriorLTILinear::evaluateError, factor, std::placeholders::_1, v1,
          p2, v2, c1, nullptr, nullptr, nullptr, nullptr, nullptr)),
      p1, 1e-6);
  expectH2 = numericalDerivative11(
      std::function<Vector(const Vector3 &)>(std::bind(
          &GPPriorLTILinear::evaluateError, factor, p1, std::placeholders::_1,
          p2, v2, c1, nullptr, nullptr, nullptr, nullptr, nullptr)),
      v1, 1e-6);
  expectH3 =
      numericalDerivative11(std::function<Vector(const Vector3 &)>(std::bind(
                                &GPPriorLTILinear::evaluateError, factor, p1,
                                v1, std::placeholders::_1, v2, c1, nullptr,
                                nullptr, nullptr, nullptr, nullptr)),
                            p2, 1e-6);
  expectH4 =
      numericalDerivative11(std::function<Vector(const Vector3 &)>(std::bind(
                                &GPPriorLTILinear::evaluateError, factor, p1,
                                v1, p2, std::placeholders::_1, c1, nullptr,
                                nullptr, nullptr, nullptr, nullptr)),
                            v2, 1e-6);
  expectH5 = numericalDerivative11(
      std::function<Vector(const Vector3 &)>(std::bind(
          &GPPriorLTILinear::evaluateError, factor, p1, v1, p2, v2,
