...
/// initialization
gtsam::Values initArmTrajStraightLine(Vector init_conf, Vector end_conf,
                                      size_t total_step);
gtsam::Values initPose2VectorTrajStraightLine(const gtsam::Pose2& init_pose,
                                              Vector init_conf,
                                              const gtsam::Pose2& end_pose,
                                              Vector end_conf,
                                              size_t total_step);
...
gtsam::Values interpolatePose2MobileArmTraj(
    const gtsam::Values& opt_values, const gtsam::noiseModel::Base* Qc_model,
...
virtual class PriorFactorPose2Vector : gtsam::NoiseModelFactor {
  PriorFactorPose2Vector(size_t poseKey, const gpmp2::Pose2Vector& value,
                         const gtsam::noiseModel::Base* model);
};

/// values utils
void insertPose2VectorInValues(size_t key, const gpmp2::Pose2Vector& p,
                               gtsam::Values& values);
